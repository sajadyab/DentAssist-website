...
$pendingRequestsRow = $db->fetchOne(
    'SELECT COUNT(*) AS c FROM appointment_requests WHERE patient_id = ?',
    [$patientId],
    'i'
);
$pendingRequestsCount = (int) ($pendingRequestsRow['c'] ?? 0);
    ...
    <div class="row queue-main-layout">
        ...
        <div class="col-12 queue-cell-sidebar order-2">
            <div class="form-card mb-4 queue-compact-form queue-registration-card" id="queue-walkin">
                <div class="card-header bg-white border-0 py-3 queue-panel-card-header">
                    <h5 class="card-title mb-0">
                        <i class="fas fa-calendar-week me-2" aria-hidden="true"></i>Weekly queue request
                    </h5>
                    <p class="small text-muted mb-0 mt-1">Request appears on your dentist’s <strong>Weekly</strong> tab in the clinic queue.</p>
                </div>
